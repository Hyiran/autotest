...
public class HTMLReporter extends AbstractReporter
{
    ...
    private static final String HTMLReporter_KEY=  "htmlreporter";
    ...
    public String nowTime(int i) {
		...
		case 3:
			...
			break;
		case 4:
			df = new SimpleDateFormat("MMddHHmmss");// 设置日期格式
			result = df.format(new Date());// new Date()为获取当前系统时间
			break;
			
		case 5:
			df = new SimpleDateFormat("ddHH");// 设置日期格式
			result = df.format(new Date());// new Date()为获取当前系统时间
			break;
		default:
                               ...
                               String outputDirectoryName)
    {
        ...
        while(itr.hasNext()){
        	ISuite is=	itr.next();
        	this.suiteName=is.getName();
        	this.REPORT_DIRECTORY=this.suiteName+nowTime(5);
        	
        	System.out.println("测试套件"+this.suiteName+"输出目录为："+this.REPORT_DIRECTORY); 
                                ...
                                boolean onlyFailures) throws Exception
    {
        ...
        context.put(HTMLReporter_KEY, this);
     ...
     * 目录创建
     */
    private void mkdirs(File fatherFile,String newdir){
    ...
    private void copyResources(File outputDirectory) throws IOException
    {
        ...
        copyClasspathResource(outputDirectory, "jquery-1.11.1.min.js", "jquery-1.11.1.min.js");
        this.mkdirs(outputDirectory, "dist\\chart");
        File dist=new File(outputDirectory,"dist");
        File chart=new File(outputDirectory,"dist\\chart");
        copyClasspathResource(dist, "echarts.js", "echarts.js");
        copyClasspathResource(chart, "pie.js", "pie.js");
        copyClasspathResource(chart, "bar.js", "bar.js");
